...
async def braitenberg_machine(robot: cozmo.robot.Robot):
	...
	while True:
		...
		#get camera image
		camera = robot.camera
		exposure = (camera.config.min_exposure_time_ms + camera.config.max_exposure_time_ms) * 0.25
		gain = (camera.config.min_gain + camera.config.max_gain) * 0.25
